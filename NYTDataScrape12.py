...
#Get articles from the 2012 election
...
def fetch_articles(): # Get and parse multiple pages of articles
    ...
    with open('obamaromney.csv', 'wb') as output_file:
...
bad_pages16 = [] #Empty list for storing pages that fail


def get_one_page_of_articles16(page): #Function that returns one speficied article page
    articles = {}
    try:
        articles = api.search( q = 'election', 
            fq = {'source':['The New York Times']},
...
def fetch_articles16(): # Get and parse multiple pages of articles
    master_articles = [] #Loop
    for i in range(1, 101):
        sleep(2) #Prevent API rate limiting
        print "On Page: %d" % i #Track page number for visiual progress
        master_articles16 = master_articles + parse_articles(get_one_page_of_articles(i), i)

    keys = master_articles16[0].keys() #Print
